...
def tower_hanoi(num: int, source: int, temp: int, destination: int):
    if num == 0:
        return

    tower_hanoi(num - 1, source, destination, temp)
    print(f"Moving {subscript(num)} disk from {subscript(source)} tower to {subscript(destination)} tower.")
    tower_hanoi(num - 1, temp, source, destination)
